...
| 门禁 | 要求 |
| --- | --- |
| 契约 | OpenAPI 路径、请求、响应、错误码和示例已定义，或明确说明该任务不需要契约。 |
| 数据 | 所需表、唯一键和状态字段已定义。 |
...
| 后端 Agent | Maven 模块、migration、domain/application/infrastructure/trigger、测试 | 契约变更、种子数据。 |
...
2. `ZXP-BE-SKEL-001` 到 `ZXP-BE-SKEL-004`。
...
3. 如果任务范围变化，同步更新任务清单说明。
4. 记录为什么没有沿用旧 Harness 行为。
